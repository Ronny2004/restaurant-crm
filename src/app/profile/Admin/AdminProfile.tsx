...
import { 
    UserCircle, Target, Activity, 
    Clock, Loader2, Calendar, 
    AlertTriangle, Users 
} from "lucide-react";
...
export function AdminProfile({ profile }: { profile: any }) {
    ...
    const [activeTab, setActiveTab] = useState<'info' | 'audit'>('audit'); // Por defecto en audit
    const [activeAdminModal, setActiveAdminModal] = useState<string | null>(null);
    ...
    const fetchAdminStats = async () => {
        try {
            ...
            // --- LÓGICA DEL ADMIN (Visión global del equipo) ---
            const { data: kitchenData } = await supabase.from('orders')
                .select('created_at, updated_at')
                .eq('status', 'served')
                .gte('created_at', startOfDay.toISOString());

            let globalAvgTime = "Calculando...";
            ...
            <div style={{ maxWidth: "1000px", margin: "2rem auto", display: "flex", flexDirection: "column", gap: "2.5rem", paddingBottom: "4rem" }}>
                ...
                <div className="glass-panel" style={{ padding: "2.5rem", display: "flex", alignItems: "center", gap: "2.5rem", flexWrap: "wrap" }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <p style={{ color: "var(--text-muted)", fontSize: "1.1rem", display: "flex", alignItems: "center", gap: "0.5rem" }}>
                            @{profile.username} • <Calendar size={16} /> Director del Sistema
                        </p>
                    </div>
                </div>
                ...
                <div>
                    <h2 style={{ fontSize: "1.5rem", marginBottom: "1.5rem", display: "flex", alignItems: "center", gap: "0.75rem" }}>
                        <Target size={24} color="var(--primary)" /> Rendimiento General
                    </h2>
